...
function renderCards() {
  ...
  const visible = state.skills.filter((skill) => {
    ...
    const haystack = [skill.name, skill.summary, skill.best_for, skill.outputs, ...skill.tags].join(' ').toLowerCase();
    return matchesCategory && (!query || haystack.includes(query));
  });
  $('#cards').innerHTML = visible.length ? visible.map((skill) => `
    <article class="card">
      <div class="card-top"><span class="category">${skill.category}</span><a class="external" href="${skill.url}" target="_blank" rel="noreferrer" aria-label="打开 ${skill.name}">↗</a></div>
      <h3>${skill.name}</h3><p>${skill.summary}</p>
      <div class="tags">${skill.tags.map((tag) => `<span class="tag">${tag}</span>`).join('')}</div>
      <div class="card-bottom"><div><strong>适合：</strong>${skill.best_for}</div><div><strong>产出：</strong>${skill.outputs}</div><div style="margin-top:10px">${skill.evidence.map((item) => `<span class="badge">${item}</span>`).join('')}</div></div>
